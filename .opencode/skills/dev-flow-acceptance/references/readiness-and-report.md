...
- independent acceptance checker score, findings, and raw evidence scope
...
Before emitting `acceptance_ready`, spawn an independent checker subagent with raw artifacts only:
...
Readiness requires checker score >= 95 and no P0/P1 finding. If lower, route back to execution, planning, or the user depending on whether the gap is implementation, artifact, or scope/baseline related.
...
9. independent acceptance checker score is >= 95 with no P0/P1 findings
...
9. independent acceptance checker score is >= 95 with no P0/P1 findings, unless the change is documentation-only with no behavior/config/test/user-visible impact
