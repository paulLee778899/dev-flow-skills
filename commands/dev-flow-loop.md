...
5. For delivery loops, require Baseline Docs Gate before implementation: loop-only requirements, high-level design, detailed design, and test plan (`test-plan.md`) artifacts with independent checker score >= 95. These are outer-loop control artifacts, not `/dev-flow` implementation documents.
...
9. Only after both gates are approved, phase-level dev-flow may auto-continue within baseline and must provide phase-level OpenSpec/opsx, TDD per task via superpowers or equivalent fallback, detailed test matrix, system-level acceptance evidence, and independent checker `phase_eval` score >= 95 with no P0/P1 finding.
