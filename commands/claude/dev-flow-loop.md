...
5. For delivery loops, require Baseline Docs Gate before implementation: loop-only requirements, high-level design, detailed design, test plan (`test-plan.md`), and test case workbook (`test-cases.xlsx`) artifacts with at least 2 independent checker subagents, `independent_checker_scores`, `independent_checker_count`, and all checker scores >= 95. These are outer-loop control artifacts, not `/dev-flow` implementation documents.
6. Require Execution Envelope Gate before phase handoff: Loop Phase DAG, `auto_continue_scope`, `dev_flow_phase_handoff`, budgets, stop conditions, and side-effect boundaries.
...
9. Only after both gates are approved, phase-level dev-flow may auto-continue within baseline and must provide phase-level OpenSpec/opsx, TDD per task via superpowers or equivalent fallback, detailed test matrix, system-level acceptance evidence, and `phase_eval` by at least 2 independent checker subagents with `phase_eval_result.independent_checker_scores`, `phase_eval_result.independent_checker_count`, all phase_eval checker scores >= 95, and no P0/P1 finding.
