...
9. Only after both gates are approved, phase-level dev-flow may auto-continue within baseline and must provide phase-level OpenSpec/opsx, TDD per task via superpowers or equivalent fallback, detailed test matrix, system-level acceptance evidence, and `phase_eval` by a checker subagent with `phase_eval_result.checker_score` recorded, checker score >= 95, and no P0/P1 finding.
