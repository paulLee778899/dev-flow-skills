...
Before presenting Phase 2 Gate, spawn a checker subagent with only the raw OpenSpec/opsx artifacts, `task-orchestration.md`, relevant Git diff/status when needed, and the requested review criteria. Do not pass the main agent's expected score or conclusions.
